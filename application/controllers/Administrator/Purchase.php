...
class Purchase extends CI_Controller
{
        ...
        $purchaseDetails = $this->db->query("
            select 
                ...
                (
                    ...
                    and prd.PurchaseReturnDetailsSize_ID = pd.Size_ID
        ...
        try {
            ...
            foreach ($data->cart as $product) {
                $returnDetails = array(
                    ...
                    'PurchaseReturnDetailsSize_ID'          => $product->Size_ID,
                ...
                $this->db->query("
                    ...
                    and Size_ID = ?
                    and branch_id = ?
                ", [$product->return_quantity, $product->Product_IDNo, $product->Size_ID, $product->PurchaseDetails_branchID]);
            }
        ...
        try {
            ...
            foreach ($oldDetails as $product) {
                $this->db->query("
                    ...
                    and Size_ID = ?
                    and branch_id = ?
                ", [$product->PurchaseReturnDetails_ReturnQuantity, $product->PurchaseReturnDetailsProduct_SlNo, $product->PurchaseReturnDetailsSize_ID, $product->PurchaseReturnDetails_brachid]);
            }
    ...
    public function addPurchase()
    {
        ...
        try {
            ...
            foreach ($data->cartProducts as $product) {
                ...
                $previousStock = $this->mt->productStock($product->productId, $product->size_id, $product->branchId);

                $inventoryCount = $this->db->query("select * from tbl_currentinventory where product_id = ? and Size_ID = ? and branch_id = ?", [$product->productId, $product->size_id, $product->branchId])->num_rows();
                if ($inventoryCount == 0) {
                    $inventory = array(
                    ...
                    $this->db->query("
                        ...
                        and Size_ID = ?
                        and branch_id = ?
                    ", [$product->quantity, $product->productId, $product->size_id, $product->branchId]);
                }
    ...
    public function updatePurchase()
    {
        ...
        try {
            ...
            foreach ($oldPurchaseDetails as $product) {
                $previousStock = $this->mt->productStock($product->Product_IDNo, $product->Size_ID, $product->PurchaseDetails_branchID);
                ...
                $this->db->query("
                    ...
                    and Size_ID = ?
                    and branch_id = ?
                ", [$product->PurchaseDetails_TotalQuantity, $product->Product_IDNo, $product->Size_ID, $product->PurchaseDetails_branchID]);
    ...
    public function getPurchaseDetails()
    {
        ...
        $saleDetails = $this->db->query("
            ...
            from tbl_purchasedetails pd
            join tbl_product p on p.Product_SlNo = pd.Product_IDNo
            join tbl_size ps on ps.Size_SlNo = pd.Size_ID
            join tbl_productcategory pc on pc.ProductCategory_SlNo = p.ProductCategory_ID
            join tbl_purchasemaster pm on pm.PurchaseMaster_SlNo = pd.PurchaseMaster_IDNo
            join tbl_supplier s on s.Supplier_SlNo = pm.Supplier_SlNo
            where pd.Status != 'd'
    ...
    public function  deletePurchase()
    {
        ...
        try {
            ...
            $purchaseDetails = $this->db->select('Product_IDNo,Size_ID,PurchaseDetails_TotalQuantity,PurchaseDetails_TotalAmount,PurchaseDetails_branchID')->where('PurchaseMaster_IDNo', $data->purchaseId)->get('tbl_purchasedetails')->result();

            foreach ($purchaseDetails as $detail) {
                $stock = $this->mt->productStock($detail->Product_IDNo, $detail->Size_ID, $detail->PurchaseDetails_branchID);
            ...
            foreach ($purchaseDetails as $product) {
                $previousStock = $this->mt->productStock($product->Product_IDNo, $product->Size_ID, $detail->PurchaseDetails_branchID);
                ...
                $this->db->query("
                    ...
                    and Size_ID = ?
                    and branch_id = ?
                ", [$product->PurchaseDetails_TotalQuantity, $product->Product_IDNo, $product->Size_ID, $detail->PurchaseDetails_branchID]);
    ...
    function addDamage()
    {
        ...
        try {
            ...
            $this->db->query("
                ...
                and Size_ID = ?
                and ci.branch_id = ?
            ", [$data->DamageDetails_DamageQuantity, $data->Product_SlNo, $data->size_id, $this->session->userdata('BRANCHid')]);
    ...
    public function updateDamage()
    {
        ...
        try {
            ...
            $this->db->query("
                ...
                and Size_ID = ?
                and ci.branch_id = ?
            ", [$oldProduct->DamageDetails_DamageQuantity, $oldProduct->Product_SlNo, $oldProduct->Size_ID, $this->session->userdata('BRANCHid')]);
    ...
    public function getPurchaseReturns()
    {
        ...
        if (isset($data->id) && $data->id != '') {
            ...
            $res['returnDetails'] = $this->db->query("
                ...
                from tbl_purchasereturndetails prd
                join tbl_product p on p.Product_SlNo = prd.PurchaseReturnDetailsProduct_SlNo
                join tbl_size s on s.Size_SlNo = prd.PurchaseReturnDetailsSize_ID
