    ...
    public function addProductTransfer()
    {
        ...
        try {
            ...
            foreach ($data->cart as $cartProduct) {
                ...
                $currentBranchInventoryCount = $this->db->query("select * from tbl_currentinventory where product_id = ? and Size_ID = ? and branch_id = ?", [$cartProduct->product_id, $cartProduct->size_id, $this->session->userdata('BRANCHid')])->num_rows();
                if ($currentBranchInventoryCount == 0) {
                    ...
                    $this->db->query("
                            ...
                            and Size_ID = ?
                            and branch_id = ?
                        ", [$cartProduct->quantity, $cartProduct->product_id, $cartProduct->size_id, $this->session->userdata('BRANCHid')]);
                }

                $transferToBranchInventoryCount = $this->db->query("select * from tbl_currentinventory where product_id = ? and Size_ID = ? and branch_id = ?", [$cartProduct->product_id, $cartProduct->size_id, $data->transfer->transfer_to])->num_rows();
                if ($transferToBranchInventoryCount == 0) {
                    ...
                    $this->db->query("
                            ...
                            and Size_ID = ?
                            and branch_id = ?
                        ", [$cartProduct->quantity, $cartProduct->product_id, $cartProduct->size_id, $data->transfer->transfer_to]);
                }
            }
    ...
    public function updateProductTransfer()
    {
        ...
        try {
            ...
            foreach ($data->cart as $cartProduct) {
                ...
                $transferToBranchInventoryCount = $this->db->query("select * from tbl_currentinventory where product_id = ? and Size_ID = ? branch_id = ?", [$cartProduct->product_id, $cartProduct->size_id, $data->transfer->transfer_to])->num_rows();
                if ($transferToBranchInventoryCount == 0) {
                    $transferToBranchInventory = array(
                        'product_id'             => $cartProduct->product_id,
                        'Size_ID'                => $cartProduct->size_id,
