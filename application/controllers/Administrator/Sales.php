        ...
        try {
            ...
            foreach ($data->cart as $cartProduct) {
                ...
                $this->db->query("
                    ...
                    and Size_ID = ?
                    and branch_id = ?
                ", [$cartProduct->quantity, $cartProduct->productId, $cartProduct->size_id, $this->session->userdata('BRANCHid')]);
            }
        ...
        try {
            ...
            foreach ($currentSaleDetails as $product) {
                $this->db->query("
                    ...
                    and Size_ID = ?
                    and branch_id = ?
                ", [$product->SaleDetails_TotalQuantity, $product->Product_IDNo, $product->Size_ID, $this->session->userdata('BRANCHid')]);
            }
        ...
        try {
            ...
            foreach ($data->cart as $product) {
                $returnDetails = array(
                    ...
                    'SaleReturnDetailsSize_ID'      => $product->Size_ID,
                ...
                $this->db->query("
                    ...
                    and Size_ID = ?
                    and branch_id = ?
                ", [$product->return_quantity, $product->Product_IDNo, $product->Size_ID, $this->session->userdata('BRANCHid')]);
            }
        ...
        try {
            ...
            foreach ($oldDetails as $product) {
                $this->db->query("
                    ...
                    and Size_ID = ?
                    and branch_id = ?
                ", [$product->SaleReturnDetails_ReturnQuantity, $product->SaleReturnDetailsProduct_SlNo, $product->SaleReturnDetailsSize_ID, $this->session->userdata('BRANCHid')]);
            }
        ...
        try {
            ...
            foreach ($returnDetails as $product) {
                $this->db->query("
                    ...
                    and Size_ID = ?
                    and branch_id = ?
                ", [$product->SaleReturnDetails_ReturnQuantity, $product->SaleReturnDetailsProduct_SlNo, $product->SaleReturnDetailsSize_ID, $this->sbrunch]);
            }
    ...
    public function getSaleReturns()
    {
        ...
        if (isset($data->id) && $data->id != '') {
            ...
            $res['returnDetails'] = $this->db->query("
                ...
                join tbl_size s on s.Size_SlNo = srd.SaleReturnDetailsSize_ID
...
<?php
    ...
    public function  deleteSales()
    {
        ...
        try {
            ...
            $saleDetails = $this->db->select('Product_IDNo, Size_ID, SaleDetails_TotalQuantity')->where('SaleMaster_IDNo', $saleId)->get('tbl_saledetails')->result();

            foreach ($saleDetails as $detail) {
                /*Get Product Current Quantity*/
                $totalQty = $this->db->where(['product_id' => $detail->Product_IDNo, 'Size_Id' => $detail->Size_ID, 'branch_id' => $sale->SaleMaster_branchid])->get('tbl_currentinventory')->row()->sales_quantity;

                /* Subtract Product Quantity form  Current Quantity  */
                $newQty = $totalQty - $detail->SaleDetails_TotalQuantity;

                /*Update Sales Inventory*/
                $this->db->set('sales_quantity', $newQty)->where(['product_id' => $detail->Product_IDNo, 'Size_ID' =>  $detail->Size_ID, 'branch_id' => $sale->SaleMaster_branchid])->update('tbl_currentinventory');
            }
